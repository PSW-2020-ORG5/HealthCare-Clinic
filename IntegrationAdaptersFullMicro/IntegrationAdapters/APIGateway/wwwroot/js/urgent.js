...
function singleKeyHttp(key, med){
...
function checkHttpNew(reqs){

    console.log("******************** usao u HTTP ***********************************");
    var name = "";
    var amount = "";
    retList = new Array();
    var ajaxList = new Array();
    ...
    reqs.forEach(function(req){     //req: potrebni lekovi/lek
        name = req.name;
        console.log(name);
        amount = req.amount
...
function  singleKeyGrpc(oneKey, req){
...
function addListenerToOrder(id){
    ...
    var subPharm = pharm.substring(0,pharm.length-1)

    allKeys.forEach(function(oneKey){
        if(oneKey.name == subPharm && oneKey.baseUrl!="" && oneKey.baseUrl!=null){
            currPort = oneKey.baseUrl;
        }
    })
